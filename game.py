...
class MoviePlot(pydantic.BaseModel):
    ...
    def strip_non_emoji(cls, value):
        ...
        value = value.replace("|", "\n")
        # strip emoji
        tokens = emoji.analyze(value, non_emoji=True)
        tokens = [
            c
            for c in tokens
            if isinstance(c.value, emoji.EmojiMatch) or (c.value == "\n")
        ]
        value = "".join(c.chars for c in tokens)
        return value
...
def gradio_interface():
    with gr.Blocks(css=".emoji-text textarea {font-size: 26px !important}") as demo:
        ...
        async def get_movie_plot(
            ...
            if not movie_order and not next_movie_plot_task:
            ...
            if movie_order:
                ...
                new_movie_plot_task = (
                    new_title,
                    asyncio.create_task(from_title(new_title)),
                )
            else:
                new_movie_plot_task = None
